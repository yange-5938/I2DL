...
class RescaleTransformTestMin(UnitTest):
    ...
    def test(self):
        ...
        for sample in tqdm(self.dataset, position=0, leave=True):
...
class RescaleTransformTestMax(UnitTest):
    ...
    def define_failure_message(self):
        return "Images were not rescaled correctly. " \
               "Expected new maximum to be %d but got %s." \
...
class CIFARImageMeanTest(UnitTest):
    ...
    def test(self):
        return np.abs(np.sum(self.mean - self.expected_mean)) < 1e-3
...
class CIFARImageStdTest(UnitTest):
    ...
    def test(self):
        return np.abs(np.sum(self.std - self.expected_std)) < 1e-3
